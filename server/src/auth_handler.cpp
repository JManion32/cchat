...
json AuthHandler::handleAuthRequest(std::shared_ptr<Client> client,
                                    const json &payload) {
  // Validate payload
  ...
  std::cout << "[AUTH] SUCCESS: user= " << client->getName()
            << " token= " << client->getToken() << std::endl;

  std::string server_msg =
      payload["name"].get<std::string>() + " joined the chat";
...
void AuthHandler::sendAuthResponse(std::shared_ptr<Client> client) {
  ...
  json auth_response = {{"type", "auth.response"},
                        {"payload",
                         {
                             {"success", true},
                             {"name", client->getName()},
