...
json ChatHandler::handleChatRequest(std::shared_ptr<Client> client,
                                    const json &payload) {

  // Validate token exists in payload
  if (!payload.contains("token") || !payload["token"].is_string()) {
    std::cerr << "[CHAT] ERROR: missing or invalid token in payload"
              << std::endl;
    return json();
  }
  ...
  if (!payload.contains("content") || !payload["content"].is_string()) {
    std::cerr << "[CHAT] ERROR: missing or invalid content in payload"
              << std::endl;
    return json();
  }
  ...
  json response = {
      {"type", "chat.response"},
      {"payload",
       {{"server", false}, {"name", client->getName()}, {"content", content}}}};
