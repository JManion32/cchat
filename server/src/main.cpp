...
void* client_thread(void* arg) {
    ...
    while (true) {
        ...
        if (bytes <= 0) {
            std::cout << "[SERVER] Client disconnected or recv error (" << bytes << ")\n";
            break;
        }
        ...
        while (true) {
            ...
            if (body_len < 5) {
                std::cerr << "[SERVER] Invalid body_len received: " << body_len << ". Closing connection.\n";
                goto disconnect;
            }
...
int main() {
    ...
    if (!socket_bind(server_fd, SERVER_PORT)) {
        std::cerr << "[SERVER] socket_bind() failed\n";
        return -1;
    }

    if (!socket_listen(server_fd, 10)) {
        std::cerr << "[SERVER] socket_listen() failed\n";
        return -1;
    }
    ...
    while (true) {
        ...
        if (client_fd < 0) {
            std::cerr << "[SERVER] socket_accept() failed\n";
            continue;
        }
