...
Client::~Client() { socket_close(sockfd); }
...
void *Client::recv_loop(void *arg) {
  ...
  while (true) {
    int bytes =
        socket_recv(self->sockfd, reinterpret_cast<char *>(temp), sizeof(temp));
  ...
  QMetaObject::invokeMethod(
      ...
      [this, msg]() {
        ...
        if (msg["type"] == "auth.response" &&
            msg["payload"].value("success", false)) {
        ...
        // CHAT RESPONSE
          ...
          // From Self
                   ...
                   QString::fromStdString(msg["payload"].value("name", ""))) {
            credit_count += 1;
...
QWidget *Client::buildLoginScreen() {
  ...
  connect(usernameInput, &QLineEdit::textChanged,
  ...
  connect(connectButton, &QPushButton::clicked, [this, usernameInput]() {
    ...
    // Build AUTH_REQUEST
    json request = {{"type", "auth.request"},
                    {"payload", {{"name", this->username.toStdString()}}}};
...
QWidget *Client::buildChatScreen() {
  ...
  // SHOP BUTTON
  shopButton =
      new QPushButton("Theme Shop (" + QString::number(credit_count) + ")");
  ...
  connect(messageBox, &QLineEdit::textChanged,
          [sendButton](const QString &text) {
  ...
  connect(sendButton, &QPushButton::clicked, [this, messageBox]() {
    ...
    // Build CHAT_REQUEST
    json request = {{"type", "chat.request"},
                    {"payload",
                     {
  ...
  connect(messageBox, &QLineEdit::returnPressed, sendButton,
          &QPushButton::click);
...
QWidget *Client::buildShopScreen() {
  ...
  std::vector<ThemeItem> themes = {
      ...
      {6, "Halloween", "../../client/themes/halloween_mode.png", "halloween",
       150},
      {7, "Festive", "../../client/themes/festive_mode.png", "festive", 200},
      {8, "Premium Gold", "../../client/themes/premium_mode.png", "premium",
       100000},
  };
  ...
  for (int i = 0; i < (int)themes.size(); i++) {
    ...
    connect(buyBtn, &QPushButton::clicked, [this, themeId, theme]() {
      ...
      json request = {{"type", "purchase.request"},
                      {"payload",
                       {
                           {"token", token.toStdString()},
                           {"index", themeId},
...
void Client::addMessage(const QString &user, const QString &text,
